...
export interface LineMatcher<T> {
  testLine(
    ...
    | { hasData: true; data: T; stop: boolean }
    | { hasData: false; stop: boolean };
}

type SearchResult<T> = Result<T[], Error>;

export default class DocumentReader
  implements IterableIterator<vscode.TextLine>
{
  ...
  public goToLine(lineNumber: number) {
    const clampedLine = Math.max(
      0,
      Math.min(lineNumber, this.document.lineCount),
    );
  ...
  searchLine<T>(step: number, matcher: LineMatcher<T>): SearchResult<T> {
    ...
    for (let lineIdx = this._currentLineIndex; lineIdx >= 0; lineIdx += step) {
      ...
      const testResult = matcher.testLine(line, (lineNum) =>
        this.document.lineAt(lineNum),
      );
  ...
  public next(): { done: boolean; value: vscode.TextLine } {
    const done = this._currentLineIndex >= this.document.lineCount;
    if (done) {
