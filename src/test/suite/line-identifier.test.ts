...
class LineIdentifierTest {
  ...
  private thenAssertLineTypeOk(type: LineType, message?: string) {
    ...
    message =
      message ||
      `Line type should be ${LineType[type]}, but was ${LineType[this.parsedLine.type]}`;
  ...
  public dateTypeOK() {
    ...
    this.lineTypeOK(
      "11/09/2001 //Unforgettable date (commenting is allowed on dates)",
      LineType.Date,
    );
  }
  ...
  public watchEntryTypeOK() {
    ...
    this.lineTypeOK(
      "04:30 - 04:52 03 {Vitinho, Gabs, Mayara}",
      LineType.WatchEntry,
    );
  ...
  public tagTypeOK() {
    ...
    this.lineTypeOK(
      '[NOT-ANIME(reason = "Filler", episodes = 40-45 )]',
      LineType.Tag,
    );
  }
  ...
  public showTitleTypeFail() {
    ...
    this.lineTypeFail(
      "Tags should not be here: [NOT-ANIME]",
      LineType.ShowTitle,
    );
    this.lineTypeFail(
      "[NOT-ANIME] Tags should not be here:",
      LineType.ShowTitle,
    );
    this.lineTypeFail(
      "Tags should [NOT-ANIME] not be here:",
      LineType.ShowTitle,
    );
    this.lineTypeFail(
      "Friends should not be here: {Friend}",
      LineType.ShowTitle,
    );
    this.lineTypeFail(
      "{Friend} Friends should not be here:",
      LineType.ShowTitle,
    );
    this.lineTypeFail(
      "Friends should not {Friend} be here:",
      LineType.ShowTitle,
    );
  ...
  public watchEntryTypeFail() {
    this.lineTypeFail(
      "20:00 - 20:01",
      LineType.WatchEntry,
      "Should fail because it is missing the episode number",
    );
    this.lineTypeFail(
      "19:14 19:44 12",
      LineType.WatchEntry,
      "Should fail because it is missing the dash",
    );
    this.lineTypeFail(
      "19:14 @ 19:44 12",
      LineType.WatchEntry,
      "Should fail because it has @ instead of dash",
    );
    this.lineTypeFail(
      "1914 - 1944 12",
      LineType.WatchEntry,
      "Should fail because it is missing the colons",
    );
    this.lineTypeFail(
      "19:14 - 1944 12",
      LineType.WatchEntry,
    ...
    this.lineTypeFail(
      "1914 - 19:44 -1",
      LineType.WatchEntry,
      "Should fail because the episode number is negative",
    );
    this.lineTypeFail(
      "1914 - 19:44 AAA",
      LineType.WatchEntry,
      "Should fail because the episode number is not a number",
    );
    this.lineTypeFail(
      "20:00 - ",
      LineType.WatchEntry,
      "Should fail because it is missing the end time",
    );
...
suite("Line Identifier Test Suite", () => {
  ...
  suite("ShowTitles", () => {
    suite("should recognize correct show titles", () =>
      suites.showTitleTypeOk(),
    );
    suite("shouldn't read malformed show titles ", () =>
      suites.showTitleTypeFail(),
    );
  });
  ...
  suite("WatchEntries", () => {
    suite("should recognize correct watch entries", () =>
      suites.watchEntryTypeOK(),
    );
    suite("shouldn't read malformed watch entries ", () =>
      suites.watchEntryTypeFail(),
    );
  });
