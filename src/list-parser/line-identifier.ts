...
export default class LineIdentifier {
  public static identifyLine(line: TextLine): LineInfo {
    ...
    const commentTokenPosition = text.indexOf(COMMENT_TOKEN);
  ...
  private static getShowTitleParams(
    line: TextLine,
  ...
  private static getWatchEntryParams(
    line: TextLine,
    groups: RegExpExecArray,
  ): LineInfo {
    const errors = [];
    ...
    return {
      ...
      params: {
        ...
        episode: groups[3],
        company: groups[4]
          ? (groups[4] as string).split(",").map((name) => name.trim())
          : [],
      },
      line,
    };
  ...
  private static getTagParams(
    line: TextLine,
    groups: RegExpExecArray,
  ): LineInfo {
    const tagName = groups[1].trim();
    const tagParamsStr = groups[2];
    ...
    if (!tagParamsStr || tagParamsStr.trim() === "") {
      ...
      tagParams = tagParamsStr.split(",").map((paramStr) => {
        const execResult = TAG_PARAM_REG.exec(paramStr);
        if (!execResult) {
          parametersValid = false;
          return <TagParam>{};
        }
        const [_, name, value] = execResult;
