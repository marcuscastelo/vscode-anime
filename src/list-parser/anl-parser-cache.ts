...
import { type ExtensionContext, Range, type TextDocument } from 'vscode';
...
export class AnlParserCacheManager {
	...
	public onCheckpoint({
		...
		console.log(
			`[marucs-anime::cache::onCheckpoint] Registering cache checkpoint for ${document.uri.toString()}, line count: ${lineCount}`
		);
	...
	public findValidCache(document: TextDocument): AnlParserCache | undefined {
		console.log(
			`[marucs-anime::cache::findValidCache] Finding valid cache for ${document.uri.toString()}`
		);
		if (this.caches.length === 0) {
			console.log('[marucs-anime::cache::findValidCache] No caches found');
			return undefined;
		}
		console.log(`[marucs-anime::cache::findValidCache] Found ${this.caches.length} caches`);

		console.log(`[marucs-anime::cache::findValidCache] Purging caches to max of ${this.maxCaches}`);
		...
		const indexOfFirstInvalidCache = this.caches.findIndex(cache => {
			...
			if (invalid) {
				console.log(
					`[marucs-anime::cache::findValidCache] Invalid cache found for ${document.uri.toString()}`
				);
			}
			return invalid;
		});
		...
		if (indexOfFirstInvalidCache === -1) {
			console.log('[marucs-anime::cache::findValidCache] All caches are valid, returning last');
		...
		console.log(`[marucs-anime::cache::findValidCache] Returning last valid cache`);
	...
	private isCacheValid(document: TextDocument, cache: AnlParserCache): boolean {
		if (document.uri.toString() !== cache.validity.documentUri) {
			console.debug(`[marucs-anime::cache::isCacheValid] Invalid! Reason: Different document`);
			return false;
		}
		...
		if (!valid) {
			console.debug(`[marucs-anime::cache::isCacheValid] Invalid! Reason: Text hash mismatch`);
		}
		return valid;
	}
	...
	private persistCaches(): void {
		...
		for (const cache of this.caches) {
			const percentage =
				(cache.validity.lineCount / this.caches[this.caches.length - 1].validity.lineCount) * 100;
			const missingLines =
				this.caches[this.caches.length - 1].validity.lineCount - cache.validity.lineCount;
			console.log(
	...
	private loadCaches() {
		...
		const stringifiedCaches = this.context.globalState.get<string>('anl-parser-cache');
		...
		this.caches = JSON.parse(stringifiedCaches) as AnlParserCache[];
		console.log(`[marucs-anime::cache::loadCaches] Loaded ${this.caches.length} caches`);
	}
}
