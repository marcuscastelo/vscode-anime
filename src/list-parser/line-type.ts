...
class GlobalRegex {
  ...
  public exec(string: string) {
    return new RegExp(this.regex_pattern).exec(string);
  }
}
...
export const WATCH_REG = makeGlobalReg(
  /^([0-9]{2}:[0-9]{2})\s*-\s*([0-9]{2}:[0-9]{2})?\s+([0-9][0-9.]{1,}|--)?\s*(?:\{(.*)\})?\s*$/,
);
export const TAG_REG = makeGlobalReg(
  /^\s*(?<!\[)\[([^=[\]0-9]+?)(?:\(([^)]+)\))?\](?!\])\s*(?:\/\/.*)?$/,
);
