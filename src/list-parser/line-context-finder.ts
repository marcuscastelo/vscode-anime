...
type SearchResults<T> =
  | { found: true; info: T[] }
  | { found: false; info: PredefinedArray<never[]> };
...
export default class LineContextFinder {
  ...
  private static findLastShowTitle(
    ...
    const showTitleRes = reader.searchLine(-1, showTitleMatcher);
  ...
  public static findLastDate(reader: DocumentReader): DateSearchResult {
    ...
    const dateRes = reader.searchLine(-1, dateMatcher);
    if (isOk(dateRes)) {
  ...
  private static findLastWatchEntry(
    ...
    const watchEntryRes = reader.searchLine(-1, watchEntryMatcher);
  ...
  private static findTags(
    ...
    const tagMatcher: LineMatcher<TagLineInfo> = {
      testLine: (line: TextLine) => {
        ...
        if (
          lineInfo.type === LineType.Ignored ||
          lineInfo.type === LineType.Invalid
        ) {
          return { hasData: false, stop: false };
        }
        ...
        if (lineInfo.type === LineType.Tag) {
          ...
          const tag = MarucsAnime.INSTANCE.tagRegistry.get(
            lineInfo.params.tagName,
          );
          if (tag === undefined) {
          ...
          if (tag.target === TagTarget.WATCH_LINE) {
            ...
            if (stage !== "wrong_show") {
              ...
              console.error(
                "Invalid state!! (trying to read tags above wrong_show)",
              );
              return { hasData: false, stop: true };
            }
          }
    ...
    const tagRes = reader.searchLine(-1, tagMatcher);
    if (isOk(tagRes)) {
  ...
  public static findContext(
    ...
    reader.goToLine(lineNumber);
    const lastWatchEntryRes = this.findLastWatchEntry(
      reader,
      lastShowTitleRes.info.params.showTitle,
    );

    //Gets last used tags for last show
    reader.goToLine(lineNumber);
    const tagsRes = this.findTags(
      reader,
      lastShowTitleRes.info.params.showTitle,
    );
    ...
    const context: LineContext = {
      ...
      lastWatchEntryLine: lastWatchEntryRes.found
        ? lastWatchEntryRes.info
        : undefined,
    };
