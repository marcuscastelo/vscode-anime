...
import { DiagnosticSeverity, Location, Range, type TextDocument, type TextLine } from 'vscode';
...
import type MADiagnosticController from '../lang/maDiagnosticCollection';
import {
	type CompleteWatchEntry,
	type DocumentContexted,
	type PartialWatchEntry,
	type WatchEntry,
} from '../types';
import { ddmmyyyToDate } from '../utils/date-utils';
import DocumentReader from '../utils/document-reader';
import { type Supplier } from '../utils/typescript-utils';
...
export default class AnlParser {
	...
	parseDocument(
		...
		console.log(`[anl-parser] Processing ${document.uri.toString()}...`);
		...
		console.log(`[anl-parser] Finished processing ${document.uri.toString()}`);
	}
	...
	private parseDateLine(lineInfo: DateLineInfo) {
		...
		if (mostRecentDateStr !== undefined) {
			...
			if (isErr(mostRecentDateRes)) {
				const diagnosticLine = this.diagnosticExtraContext.mostRecentDateLine?.line.lineNumber;
		...
		if (
			...
			newDate.getTime() >
				equip(ddmmyyyToDate(this.diagnosticExtraContext.mostRecentDateLine.params.date))
					.unwrap()
					.getTime()
		) {
	...
	private parseShowTitleLine(lineInfo: ShowTitleLineInfo, document: TextDocument) {
		...
		this.lineContext.currentTagsLines = this.lineContext.currentTagsLines?.filter(
			lineInfo =>
		...
		if (isErr(showResult)) {
			this.diagnosticController.addLineDiagnostic(
				lineInfo.line,
				`Error while processing show: ${showResult.data.message}`
			);
			return;
		}
		...
		const messageBitmask = (missingTags.length > 0 ? 1 : 0) | (extraTags.length > 0 ? 2 : 0);
		if (messageBitmask !== 0) {
	...
	private parseWatchLine(lineInfo: WatchEntryLineInfo) {
		...
		const currentShow = this.getShowRegistry().searchShow(currentShowLine.params.showTitle);
		...
		if (!validTimeReg.test(endTime)) {
			this.diagnosticController.addRangeDiagnostic(
				new Range(lineStart.with({ character: 6 }), lineStart.with({ character: 10 })),
		...
		const lastWatchedEpisode = currentShow.lastCompleteWatchEntry?.data.episode ?? 0;
	...
	private parseTagLine(lineInfo: TagLineInfo, reader: DocumentReader) {
		...
		if (!tag) {
			this.diagnosticController.addLineDiagnostic(lineInfo.line, 'Unknown tag, ignoring!', {
				severity: DiagnosticSeverity.Warning,
			});
			return;
		}
		...
		if (tag.name === 'SCRIPT-SKIP') {
			const paramValue = lineInfo.params.tagParams.find(tp => tp.name === 'count')?.value;
		...
		if (tag.target !== TagTarget.SCRIPT_TAG) {
			if (
				this.lineContext.currentTagsLines?.map(lineInfo => lineInfo.params.tag).indexOf(tag) === -1
			) {
