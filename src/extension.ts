...
import { type LineInfo } from './list-parser/line-info';
...
export class MarucsAnime {
	...
	get context() {
		return (this.activationState.activated && this.activationState.context) || undefined;
	}
	...
	public reactToDocumentChange(context: ExtensionContext, document: vscode.TextDocument) {
		...
		const needsParsing = !cache || cache.validity.lineCount !== document.lineCount;

		if (needsParsing) {
			if (!cache) {
				console.log('[marucs-anime::cache] No valid cache found, parsing document');
			} else if (cache.validity.lineCount !== document.lineCount) {
				console.log('[marucs-anime::cache] Incomplete cache found, parsing document');
			}
	...
	public scanDocument(_context: ExtensionContext, document: vscode.TextDocument) {
		...
		vscode.window.setStatusBarMessage(`Parsing completed! Cache count: ${currentCacheCount}`);
	}
	...
	private updateShowRegistry(textDocument: TextDocument) {
		...
		const parser = new AnlParser(showRegistrySupplier, this.diagnosticController);
		...
		parser.parseDocument(textDocument, {
			...
			listener: (lineInfo: LineInfo) => {
				...
				const checkpoints = Array.from({ length: MAX_CACHE_SIZE }, (_, i) => lineOfCheckpoint(i));

				const dedupedCheckpoints = Array.from(new Set(checkpoints));

				if (dedupedCheckpoints.includes(line)) {
					console.log(`[marucs-anime] Registering parsing checkpoint at ${line}`);
	...
	private registerSubscriptions(context: ExtensionContext) {
		context.subscriptions.push(
			vscode.window.onDidChangeActiveTextEditor(
				editor => editor && this.reactToDocumentChange(context, editor.document)
			),
			...
			vscode.commands.registerTextEditorCommand('marucs-anime.clearCache', clearCache),
