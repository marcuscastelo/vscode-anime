...
async function searchMAL(animeTitle: string) {
  ...
  if (
    foundAnimes[0].titles.filter(
      (title) => title.title.toLowerCase() === animeTitle.toLowerCase(),
    ).length === 0
  ) {
    return undefined;
  }
...
export default class ShowHoverProvider implements HoverProvider {
  ...
  private generateLineContextMD(document: TextDocument, line: number) {
    ...
    const tagNames = currentTagsLines.map(
      (lineInfo) => lineInfo.params.tag.name,
    );
    ...
    if (show) {
      ...
      if (isOk(firstMentionedContext)) {
        showLastTitle =
          firstMentionedContext.data.currentShowLine?.params.showTitle ||
          "EMPTY";
        showLastDate =
          firstMentionedContext.data.currentDateLine?.params.date || "EMPTY";
        showLastTagNamesString = firstMentionedContext.data.currentTagsLines
  ...
  public async provideHover(
    ...
    const malData = await searchMAL(
      lineContext.data.currentShowLine.params.showTitle,
    );
