...
async function searchMAL(animeTitle: string) {
	...
	if (
		foundAnimes[0].titles.filter(title => title.title.toLowerCase() === animeTitle.toLowerCase())
			.length === 0
	) {
		return undefined;
	}
...
export default class ShowHoverProvider implements HoverProvider {
	...
	private generateLineContextMD(document: TextDocument, line: number) {
		...
		const show = MarucsAnime.INSTANCE.showRegistry.searchShow(currentShowLine.params.showTitle);
	...
	public async provideHover(document: TextDocument, position: Position, _token: CancellationToken) {
		...
		const lineContextMD = new MarkdownString(`![MAL](${malData?.images.jpg.image_url})`);
