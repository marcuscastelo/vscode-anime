...
const defaultWatchSessionTagsNames = ["勉強", "REWATCH"];

const defaultWatchLineTagsNames = ["UNSAFE-ORDER"];
...
function createDefaultTags(): Tag[] {
  ...
  const watchSessionTags = createTagsFromNames(
    defaultWatchSessionTagsNames,
    TagTarget.WATCH_SESSION,
  );
  const watchLineTags = createTagsFromNames(
    defaultWatchLineTagsNames,
    TagTarget.WATCH_LINE,
  );
